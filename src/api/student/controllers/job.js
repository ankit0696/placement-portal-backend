...
module.exports = {
  ...
  async get_eligible_jobs(ctx) {
    ...
    if (approved !== "approved") {
      return ctx.badRequest(null, [
        { messages: [{ id: "Account not approved yet" }] },
      ]);
    }
    ...
    if (!eligible_jobs || !Array.isArray(eligible_jobs)) {
      return ctx.internalServerError(null, [
        { messages: [{ id: "Could not get eligible jobs" }] },
      ]);
    }
    ...
    const exists = await Promise.all(
      eligible_jobs.map(async (job) => {
        const existing_application = await strapi.db
  ...
  async apply_to_job(ctx) {
    ...
    if (!query || !query.jobId) {
      return ctx.badRequest(null, [
        { messages: [{ id: "Required jobId in query" }] },
      ]);
    }
    ...
    if (!job) {
      return ctx.badRequest(null, [
        { messages: [{ id: "No such job Id found" }] },
      ]);
    }

    if (
      X_marks >= job.min_X_marks &&
      XII_marks >= job.min_XII_marks &&
      Date.now() <= job.last_date
    ) {
      const existing_application = await strapi.db
        .query("api::application.application")
        .findOne({
      ...
      if (!application) {
        return ctx.internalServerError(null, [
          { messages: [{ id: "Failed to create application" }] },
        ]);
      }
  ...
  async get_applied_jobs(ctx) {
    ...
    const applied_jobs = await strapi.db
      .query("api::application.application")
      .findMany({
        where: {
          student: id,
